...
import { GET, PUT, DELETE } from '@/app/api/textadv/[id]/route';
...
vi.mock('@/cash/TextAdvCash', () => ({
   default: [],
}));

describe('TextAdv [id] Route', () => {
   let mockGet: any;
   let mockPut: any;
   let mockDelete: any;

   const params = { params: { id: '606' } };
   ...
   it('should call handler.Get with id', async () => {
      ...
      expect(RequestHandler).toHaveBeenCalledWith(TextAdvModel, TextAdvCash);
      expect(mockGet).toHaveBeenCalledWith('606');
   ...
   it('should call handler.PUT with id and request', async () => {
      const req = new Request('http://localhost', {
         method: 'PUT',
         body: JSON.stringify({ formData: { content: 'Updated ad text' } }),
      });
   ...
   it('should handle missing id in DELETE', async () => {
      ...
      expect(mockDelete).toHaveBeenCalledWith('', 'Ads Deleted successfully');
   });
});
