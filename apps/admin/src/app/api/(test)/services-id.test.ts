...
vi.mock('@/cash/ServicesCash', () => ({
   default: [],
}));

describe('Services [id] Route', () => {
   let mockGet: any;
   let mockPut: any;
   let mockDelete: any;

   const params = { params: { id: '505' } };
   ...
   it('should call handler.Get with id', async () => {
      ...
      expect(RequestHandler).toHaveBeenCalledWith(ServiceModel, ServicesCash);
      expect(mockGet).toHaveBeenCalledWith('505');
   ...
   it('should call handler.PUT with id and request', async () => {
      const req = new Request('http://localhost', {
         method: 'PUT',
         body: JSON.stringify({ formData: { title: 'Updated Service' } }),
      });
