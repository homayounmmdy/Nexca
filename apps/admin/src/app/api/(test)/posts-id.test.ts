...
describe('Posts [id] Route', () => {
   ...
   it('should call handler.Get with id', async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith('707');
   ...
   it('should call handler.PUT with id and request', async () => {
      const req = new Request('http://localhost', {
         method: 'PUT',
         body: JSON.stringify({ formData: { title: 'Updated Post' } }),
      });
   ...
   it('should handle missing id in DELETE', async () => {
      ...
      expect(mockDelete).toHaveBeenCalledWith('', 'Post Deleted successfully');
   });
});
