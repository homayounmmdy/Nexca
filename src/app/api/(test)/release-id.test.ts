...
import { GET, PUT, DELETE } from '@/app/api/release/[id]/route';
...
describe('Release [id] Route', () => {
   let mockGet: any;
   let mockPut: any;
   let mockDelete: any;

   const params = { params: { id: '404' } };
   ...
   it('should call handler.Get with id', async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith('404');
   ...
   it('should call handler.PUT with id and request', async () => {
      const req = new Request('http://localhost', {
         method: 'PUT',
         body: JSON.stringify({ formData: { version: 'v1.3.0' } }),
      });
